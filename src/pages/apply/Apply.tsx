...
const Apply: React.FC = () => {
  ...
  const handleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const formDataSerialized = formDataEntries
        .map(([fieldName, value]) => `${encodeURIComponent(fieldName)}=${encodeURIComponent(value)}`)
        .join('&');
...
<div id='apply_div3'> <div id='apply_div3_title'>
  {t("apply.please_check1")} <span id='apply_div3_title_gray'>{t("apply.please_check2")}</span>
  </div>
  ...
  <div className='apply_check_div'><input type='checkbox' className='apply_checkbox'/><span>I study at university</span></div>
    <div className='apply_check_div'><input type='checkbox' className='apply_checkbox'/><span>I live in Chisinau</span></div>
      ...
      <form id="apply_form" onSubmit={handleSubmit} method="POST">
          ...
          <input className="apply_form_group"
                 id="274219221"
                 type="text"
        ...
        <input id='apply_send_btn' className="btn btn-primary" type="submit" value='Apply' />
      </form>
    </div>
